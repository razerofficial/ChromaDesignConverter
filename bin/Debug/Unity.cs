...
float t = 1 - Math.Floor(Math.Cos(speed * Math.PI * (frameId + 1) / (float)frameCount));
...
float t = Math.Floor(Math.Cos(speed * Math.PI * (frameId + 1) / (float)frameCount));
...
for (int i = 0; i < 360; ++i) {
int angle = i * Math.PI / 180;
int r = floor(startRow + stroke * Math.sin(angle));
int c = floor(startColumn + stroke * Math.Cos(angle));
if (r >= 0 && r < maxRow &&
c >= 0 && c < maxColumn) {
int key = (r << 8) | c;
...
for (int frameId = 0; frameId < frameCount; ++frameId) {
t += speed;
int hp = Math.Floor(Math.Cos(Math.PI / 2 + t));
